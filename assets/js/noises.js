...
async function loadSounds() {
  const response = await fetch("sounds.json") const json = await response.json() return json;
}
...
function addSoundsToPage(sounds) {
  sounds.forEach(sound => {
    ...
    const player = document.createElement("audio") soundsElement.appendChild(player) player.setAttribute("src", `assets/audio/${sound.src}`) soundButton.addEventListener('click', () => {
      player.play();
    });
